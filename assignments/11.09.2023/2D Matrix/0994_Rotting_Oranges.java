    ...
    private void rotAdjacent(int[][] grid, int i, int j, int minutes) {
        if(i < 0 || i >= grid.length /* out of bounds */
          ...
          || (1 < grid[i][j] && grid[i][j] < minutes) /* this orange is already rotten by another rotten orange */
            ...
            grid[i][j] = minutes;
            rotAdjacent(grid, i - 1, j, minutes + 1);//Check Up
            rotAdjacent(grid, i + 1, j, minutes + 1);//Check Down
            rotAdjacent(grid, i, j - 1, minutes + 1);//Check left
            rotAdjacent(grid, i, j + 1, minutes + 1);//Check right
        }
    }
}